#!/bin/bash
# Run inference for the DEEPESD_BG run. Reads inference.yaml from this directory and
# writes predictions under ./outputs/predictions/. Requires a trained
# checkpoint under ./outputs/models/ (produced by train.sh).
#
# The recipe's run_ID must equal this directory's name (DEEPESD_BG) and its
# output_dir must be this directory's parent (id_dir = output_dir/run_ID).
#
#SBATCH --job-name=deepesd_bg_infer
