...
1. An **EDM-preconditioned diffusion model** generates the **residual** `r = y − ŷ` conditioned on the low-res predictors *and* on the regression mean `ŷ`. The final downscaled prediction is `ŷ + r̂`.
...
1. On first run, computes residuals on every training/validation date and caches them to a Zarr file (subsequent runs skip this step).
1. Serves `(r, c_low, c_high)` triples to the EDM-preconditioned SongUNet.
...
inference_params:
  batch_size: 4         # number of dates whose Heun chains run in parallel on the GPU
  amp_dtype: null       # 'bfloat16' (Ampere+) / 'float16' / null
  compile: false        # torch.compile the EDM model (regressor is one-shot, not compiled)
```
