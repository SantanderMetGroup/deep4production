...
1. `d4p-inspect`: Inspects the created Zarr file for basic QA/QC.
1. `d4p-train`: Trains a deep learning downscaling model.
1. `d4p-downscale`: Runs inference using trained model.
...
1. Select and align variables across files
1. Subset the requested time range
1. Compute summary statistics (e.g., mean, standard deviation)
1. Store everything in a **Zarr dataset optimized for deep learning workflows**
...
1. Applies normalization using precomputed statistics
1. Builds the specified deep learning model (here: DeepESD)
1. Trains the model over multiple epochs
1. Monitors validation performance
1. Saves the best-performing model (or selected models) and training logs
...
1. Reads the input predictor data from Zarr files
1. Applies the same preprocessing and normalization used during training
1. Runs the model in inference mode
1. Optionally generates multiple realizations (ensemble)
1. Saves the predictions to disk (typically in NetCDF format)
