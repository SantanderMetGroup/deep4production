...
class pydataset(Dataset):
    ...
    """
    def __init__(self, predictors: dict, predictands: dict, temporal_period: list, load_in_memory: bool = True, forcings={}, cache_mb: int = None):
        # --- Parameters (X, Y) ---
        ...
        self.x, self.vars_x, self.idx_vars_x, self.operator_x, self.H_x, self.W_x, self.G_x = self.get_data_info(path_predictors, variables_predictors, operator_predictors, fmt=fmt_x, cache_mb=cache_mb)
        self.y, self.vars_y, self.idx_vars_y, self.operator_y, self.H_y, self.W_y, self.G_y = self.get_data_info(path_predictands, variables_predictands, operator_predictands, fmt=fmt_y, cache_mb=cache_mb)
        self.forcings = forcings
        if forcings:
            _, self.vars_f, self.idx_vars_f, self.operator_f, __, ___, _____ = self.get_data_info(path_predictands, variables_forcings, operator_forcings, fmt=fmt_f, cache_mb=cache_mb)
        else:
            self.vars_f = None
        ...
        if load_in_memory: # If dataset fits in memory, load all predictors to speed up
            ...
            self.data = {"x": [x["data"] for x in self.x], "y": [y["data"] for y in self.y]}
    ...
    # -------------------------------------------------------------------------
    @staticmethod
    def _resolve_normalizer_info(normalizer_info_recipe, vars, predictand=False, forcing=False):
        """
    ...
    # -------------------------------------------------------------------------
    def get_data_info(self, path_data, variables, operator_info, fmt="auto", cache_mb=None):
        """
        ...
        # --- Variables ---
        log.warning("Variable subsetting assumes the same variable order across all zarr files (e.g. self.x[0] and self.x[1]).")
        ...
        if operator_info is not None:
            ...
            log.debug("Operator for variables %s: %s", vars, operator["operator_func_per_variable"])
    ...
    def get_target_samples(self):
        ...
        for idx in range(len(self)):
            ...
            # --- Prepare target ---
            y = self.preprocess(target_date, self.data["y"], self.idx_vars_y, self.sample_map_y, ops=None, transform_to_2D=None, H=None, W=None).unsqueeze(0).cpu().numpy() # Add time dimension
            # --- To xarray ---
            lats, lons = self.get_coords()
            ds = from_pred_to_xarray(data_pred=y, time_pred=np.datetime64(target_date), vars=self.vars_y, lats=lats, lons=lons)
    ...
    # -------------------------------------------------------------------------
    def preprocess(self, date, data, idx_vars, sample_map, ops=None, transform_to_2D=False, H=None, W=None):
        """
    ...
    def __getitem__(self, idx):
        ...
        if len(dates) > 1:
            x = []
            for date in dates:
                x.append(self.preprocess(date, self.data["x"], self.idx_vars_x, self.sample_map_x, ops=self._ops_x, transform_to_2D=self.transform_to_2D_x, H=self.H_x, W=self.W_x))
            x = torch.stack(x)
        else:
            x = self.preprocess(target_date, self.data["x"], self.idx_vars_x, self.sample_map_x, ops=self._ops_x, transform_to_2D=self.transform_to_2D_x, H=self.H_x, W=self.W_x)
        # ---
        y = self.preprocess(target_date, self.data["y"], self.idx_vars_y, self.sample_map_y, ops=self._ops_y, transform_to_2D=self.transform_to_2D_y, H=self.H_y, W=self.W_y)
        # --- Forcings (f) ---
        if self.forcings:
            f = self.preprocess(target_date, self.data["y"], self.idx_vars_f, self.sample_map_y, ops=self._ops_f, transform_to_2D=self.transform_to_2D_y, H=self.H_y, W=self.W_y)
        else:
            f = "N/A"
        # --- Return ---
        return x, y, f