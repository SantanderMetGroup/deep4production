...
class DeepESD(torch.nn.Module):
    ...
    def __init__(
        ...
        if f_shape is not None:
            ...
            self.mlp_forcing = torch.nn.Linear(
                in_features=input_forcing_features, out_features=flatten_features
            )
        ...
        if self.loss_function_name == "NLLGaussianLoss":
            self.out_mean = torch.nn.Linear(
                in_features=number_neurons_last_hidden,
                out_features=number_neurons_output,
            )
            self.out_log_var = torch.nn.Linear(
                in_features=number_neurons_last_hidden,
                out_features=number_neurons_output,
            )
            ...
            self.log_shape = torch.nn.Linear(
                in_features=number_neurons_last_hidden,
                out_features=number_neurons_output,
            )
            self.log_scale = torch.nn.Linear(
                in_features=number_neurons_last_hidden,
                out_features=number_neurons_output,
            )
        else:
    ...
    def forward(self, x: torch.Tensor, f: None) -> torch.Tensor:
        ...
        if self.loss_function_name == "NLLGaussianLoss":
            mean = self.out_mean(x).view(
                B, self.num_output_vars, 1, *self.spatial
            )  # (batch_size, channel, 1, *spatial)
            log_var = self.out_log_var(x).view(
                B, self.num_output_vars, 1, *self.spatial
            )  # (batch_size, channel, 1, *spatial)
            out = torch.cat(
                (mean, log_var), dim=2
            )  # (batch_size, channel, parameters=[mean, log_var], *spatial)
        elif self.loss_function_name == "NLLBerGammaLoss":
            p = self.p(x).view(
                B, 1, *self.spatial
            )  # (batch_size, num_output_vars, *spatial)
            p = torch.sigmoid(p)
            log_shape = self.log_shape(x).view(
                B, 1, *self.spatial
            )  # (batch_size, num_output_vars, *spatial)
            log_scale = self.log_scale(x).view(
                B, 1, *self.spatial
            )  # (batch_size, num_output_vars, *spatial)
            ...
            out_default = x.view(
                B, self.num_output_vars, *self.spatial
            )  # (batch_size, num_output_vars, *spatial)
