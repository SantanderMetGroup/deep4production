...
class DeepESDDropout(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        self.dropout_conv1 = nn.Dropout2d(p=0.2)  # 20% neuronas apagadas
        self.dropout_conv2 = nn.Dropout2d(p=0.2)
        self.dropout_conv3 = nn.Dropout2d(p=0.2)

        # Dropout antes de la salida
...
class DeepESDNoisy(torch.nn.Module):
    ...
    def __init__(self, 
                 ...
                 loss_function_name: str=None,
                 output_activation: dict = None,
                 num_channels_noise: int=10,
                 members_for_training: int=10,
                 noise_percentage: list[float] = [0.8, 1.0, 0.4]):
        ...
        ## --- Hidden layers ---
        self.num_noise_channels_1 = int(np.ceil(noise_percentage[0]* num_channels_noise))
        self.conv_1 = torch.nn.Conv2d(in_channels=num_input_vars + self.num_noise_channels_1,
                                      out_channels=filters[0],
                                      kernel_size=kernel_size,
                                      padding=impute_padding(kernel_size, dilation=1))

        self.num_noise_channels_2 = int(np.ceil(noise_percentage[1]* num_channels_noise))
        self.conv_2 = torch.nn.Conv2d(in_channels=filters[0] + self.num_noise_channels_2,
                                      out_channels=filters[1],
                                      kernel_size=kernel_size,
                                      padding=impute_padding(kernel_size, dilation=1))
        
        self.num_noise_channels_3 = int(np.ceil(noise_percentage[2]* num_channels_noise))
        self.conv_3 = torch.nn.Conv2d(in_channels=filters[1] + self.num_noise_channels_3,
                                      out_channels=filters[2],
                                      kernel_size=kernel_size,
                                      padding=impute_padding(kernel_size, dilation=1))
    ...
    def forward(self, x: torch.Tensor, f: torch.Tensor | None = None):

        is_ensemble_mode = self.training or torch.is_grad_enabled()

        print(f"Values of is_ensemble_mode: {is_ensemble_mode}, self.training: {self.training}, torch.is_grad_enabled(): {torch.is_grad_enabled()}")
        print(f"X value in forward: {np.max(x.detach().cpu().numpy())}, {np.min(x.detach().cpu().numpy())}, {x.shape}")
        if is_ensemble_mode:
            members = self.members_for_training
        else:
            members = 1

        outputs = []
        for _ in range(members):
            outputs.append(self._forward_single(x, f))

        print(f"Output sizes in forward: {[o.shape for o in outputs]}")
        if is_ensemble_mode:
            print(f"Output list in ensemble mode: {outputs}")
            return outputs
        else:
            return outputs[0]
        
    def _forward_single(self, x: torch.Tensor, f: torch.Tensor | None) -> torch.Tensor:
        B = x.size(0)

        # ----- Inject noise before conv1 -----
        noise_1 = torch.randn(
            B, self.num_noise_channels_1,
            x.shape[2], x.shape[3],
            device=x.device
        )
        x = torch.cat((x, noise_1), dim=1)
        x = self.conv_1(x)
        x = torch.relu(x)

        # ----- Inject noise before conv2 -----
        noise_2 = torch.randn(
            B, self.num_noise_channels_2,
            x.shape[2], x.shape[3],
            device=x.device
        )
        x = torch.cat((x, noise_2), dim=1)
        x = self.conv_2(x)
        x = torch.relu(x)

        # ----- Inject noise before conv3 -----
        noise_3 = torch.randn(
            B, self.num_noise_channels_3,
            x.shape[2], x.shape[3],
            device=x.device
        )
        x = torch.cat((x, noise_3), dim=1)
        x = self.conv_3(x)
        x = torch.relu(x)
        x = torch.flatten(x, start_dim=1)
        ...
        # ----- Output head (tu lógica original intacta) -----
        if self.loss_function_name == "NLLGaussianLoss":
            mean = self.out_mean(x).view(B, self.num_output_vars, 1, *self.spatial)
            log_var = self.out_log_var(x).view(B, self.num_output_vars, 1, *self.spatial)
            out = torch.cat((mean, log_var), dim=2)

        elif self.loss_function_name == "NLLBerGammaLoss":
            p = torch.sigmoid(self.p(x)).view(B, 1, *self.spatial)
            log_shape = self.log_shape(x).view(B, 1, *self.spatial)
            log_scale = self.log_scale(x).view(B, 1, *self.spatial)
            out = torch.cat((p, log_shape, log_scale), dim=1)

        else:
            x = self.out(x)
            out_default = x.view(B, self.num_output_vars, *self.spatial)
