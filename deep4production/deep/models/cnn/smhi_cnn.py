...
class ConvWithLearnableMapsBlock(nn.Module):
    ...
    def __init__(
        ...
        # Learnable maps
        self.pre_maps = nn.Parameter(
            torch.zeros(1, num_pre_maps, img_shape[0], img_shape[1])
        )
...
class PixelShuffleBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.use_post_map:
            ...
            self.post_maps = nn.Parameter(
                torch.zeros(
                    1,
                    out_channels // (upscale_factor**2),
                    img_shape[0] * upscale_factor,
                    img_shape[1] * upscale_factor,
                )
            )
        else:
            ...
            self.pre_conv = nn.Conv2d(
                num_pre_maps, out_channels // (upscale_factor**2), kernel_size=1
            )
...
class SMHICNN(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        for i in dilated_layer_indices:
            ...
            conv = ConvWithLearnableMapsBlock(
                in_channels=in_ch,
        ...
        self._activation_map = {
            i: nn.Identity() for i in range(self.C_out)
        }  # default: linear
    ...
    def forward(self, x):
        ...
        x = F.interpolate(
            x, size=tuple(self.spatial_dims_out), mode="bilinear", align_corners=False
        )

        # Output layers
        if self.loss_function_name == "NLLGaussianLoss":
            mean = x[:, : self.C_out, ...].view(
                B, self.C_out, 1, *self.spatial_dims_out
            )  # (batch_size, channel, 1, *spatial)
            log_var = x[:, self.C_out :, ...].view(
                B, self.C_out, 1, *self.spatial_dims_out
            )  # (batch_size, channel, 1, *spatial)
            out = torch.cat((mean, log_var), dim=2)
        elif self.loss_function_name == "NLLBerGammaLoss":
            p = x[:, 0, ...].view(
                B, 1, *self.spatial_dims_out
