...
class UpLayer(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, trans_conv: bool):
        ...
        if trans_conv:
            self.layer_op = nn.ConvTranspose2d(
...
class abad_unet(torch.nn.Module):
    ...
    def __init__(
        ...
        ## --- Predictor checks ---
        if len(x_shape) != 3:
            error_msg = "X must have a dimension of length 3: (C, H, W)"
            raise ValueError(error_msg)
        self.num_input_vars, *self.spatial = x_shape

        ### --- Predictand checks ---
        if len(y_shape) != 3:
            error_msg = "Y must have a dimension of length 3: (C, H, W)"
            raise ValueError(error_msg)
        ...
        ## --- Decoder ---
        self.trans_conv_1 = UpLayer(
            in_channels=base_channels * 8,
            out_channels=base_channels * 4,
            trans_conv=self.trans_conv,
        )
        self.up_conv_1 = UnitConv(
            in_channels=base_channels * 8,
            out_channels=base_channels * 4,
            kernel_size=self.kernel_size,
            padding=self.padding,
        ...
        self.trans_conv_2 = UpLayer(
            in_channels=base_channels * 4,
            out_channels=base_channels * 2,
            trans_conv=self.trans_conv,
        )
        self.up_conv_2 = UnitConv(
            in_channels=base_channels * 4,
            out_channels=base_channels * 2,
            kernel_size=self.kernel_size,
            padding=self.padding,
        ...
        self.trans_conv_3 = UpLayer(
            in_channels=base_channels * 2,
            out_channels=base_channels,
            trans_conv=self.trans_conv,
        )
        self.up_conv_3 = UnitConv(
            in_channels=base_channels * 2,
            out_channels=base_channels,
            kernel_size=self.kernel_size,
            padding=self.padding,
        ...
        if self.loss_function_name == "NLLGaussianLoss":
            ...
            self.log_shape = nn.Conv2d(
                in_channels=base_channels, out_channels=1, kernel_size=1
            )
        ...
        self._activation_map = {
            i: nn.Identity() for i in range(self.num_output_vars)
        }  # default: linear
        if output_activation:
            for var_name, spec in output_activation.items():
                idx = spec["idx"]
                act_class = get_func_from_string(
                    spec.get("module", "torch.nn"),
                    spec["name"],
                    kwargs=spec.get("kwargs", None),
                )
                act = (
                    act_class if isinstance(act_class, torch.nn.Module) else act_class()
                )
    ...
    def forward(self, x):
        ...
        ## --- Output layers ---
        if self.loss_function_name == "NLLGaussianLoss":
            mean = self.out_mean(x_out).view(
                B, self.num_output_vars, 1, *self.spatial
            )  # (batch_size, channel, 1, *spatial)
            log_var = self.out_log_var(x_out).view(
                B, self.num_output_vars, 1, *self.spatial
            )  # (batch_size, channel, 1, *spatial)
            ...
            p = torch.sigmoid(p)
            log_shape = self.log_shape(x_out).view(
                B, 1, *self.spatial
            )  # (batch_size, 1, *spatial)
            log_scale = self.log_scale(x_out).view(
                B, 1, *self.spatial
            )  # (batch_size, 1, *spatial)
