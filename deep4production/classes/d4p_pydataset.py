...
class d4p_pydataset(Dataset):
  ...
  def get_statistics_from_zarr_file(self, zarr_file_path, var, axis=0):
    """
    Extract mean and std statistics from a Zarr file for specified variables,
    reshaping them to match the input data dimensions.
    
    Args:
        zarr_file_path (str): Path to the Zarr file.
        vars (list[str]): List of variable names to extract statistics for.
        axis (int): Axis to expand for broadcasting (default: 0).
    
    Returns:
        dict: Dictionary with keys as variable names and values as {"mean": ..., "std": ...}.
    """
