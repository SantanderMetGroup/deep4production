...
class AnemoiZarrStore:
    ...
    def __contains__(self, key):
        if key in ("data", "dates", "latitudes", "longitudes",
                   "mean", "std", "min", "max"):
            return True
...
def open_zarr_store(path, fmt="auto", member=0, cache_mb=None):
    ...
    if cache_mb is not None:
        ...
        cached_store = zarr.LRUStoreCache(base.store, max_size=int(cache_mb) * 1024 * 1024)
...
def zarr_inspect(zarr_path: str, fmt: str = "auto"):
    ...
    for var, idx in sorted(variables.items(), key=lambda kv: kv[1]):
        ...
        print(f"{var:22} | {m:>10} | {s:>10} | {mn:>10} | {mx:>10} | {ct:>8} | {cf:>8} | {nf:>14} | {nd:>16} | {unts:>10}")
